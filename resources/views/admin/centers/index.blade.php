...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Gestion des Centres</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="bg-white shadow-md rounded-lg p-5 mb-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <a href="{{ route('admin.centers.create', ['locale' => app()->getLocale()]) }}" class="inline-flex items-center justify-center px-5 py-2.5 bg-[#4CA3DD] hover:bg-[#2A7AB8] text-white font-medium rounded-lg transition-colors duration-200 shadow-md hover:shadow-lg">
        ...
        <div class="stats-container grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="stats-card bg-white rounded-lg p-4 shadow border border-gray-200 flex flex-col">
                ...
                <div class="stats-content">
                    <h3 class="stats-value text-2xl font-bold text-gray-800">{{ $centers->count() }}</h3>
                    <p class="stats-label text-sm text-gray-600">Centres</p>
                </div>
            ...
            <div class="stats-card bg-white rounded-lg p-4 shadow border border-gray-200 flex flex-col">
                ...
                <div class="stats-content">
                    <h3 class="stats-value text-2xl font-bold text-gray-800">{{ $centers->where('is_active', true)->count() }}</h3>
                    <p class="stats-label text-sm text-gray-600">Centres actifs</p>
                </div>
                <div class="stats-trend mt-3">
                    @php
                        $activePercentage = ($centers->count() > 0) ? ($centers->where('is_active', true)->count() / $centers->count()) * 100 : 0;
                    @endphp
                    <div class="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
            ...
            <div class="stats-card bg-white rounded-lg p-4 shadow border border-gray-200 flex flex-col">
                ...
                <div class="stats-content">
                    <h3 class="stats-value text-2xl font-bold text-gray-800">{{ $centers->sum('nb_students') }}</h3>
                    <p class="stats-label text-sm text-gray-600">Étudiants inscrits</p>
                </div>
                <div class="stats-trend mt-3">
                    <div class="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
            ...
            <div class="stats-card bg-white rounded-lg p-4 shadow border border-gray-200 flex flex-col">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="text-sm text-gray-500">Académies</span>
                </div>
                <div class="stats-content">
                    @php
                        $academiesCount = $centers->pluck('academy_id')->unique()->count();
                    @endphp
                    <h3 class="stats-value text-2xl font-bold text-gray-800">{{ $academiesCount }}</h3>
                    <p class="stats-label text-sm text-gray-600">Académies représentées</p>
                </div>
                <div class="stats-trend mt-3">
                    <div class="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
        ...
        <div class="mb-6">
            <div class="flex flex-col lg:flex-row gap-4 items-center justify-between">
                <div class="relative w-full lg:w-80">
                    ...
                    <input type="search" id="search-centers" class="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-300 rounded-lg focus:ring-[#4CA3DD] focus:border-[#4CA3DD]" placeholder="Rechercher un centre...">
                </div>
                <div class="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                    <select id="filter-academy" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#4CA3DD] focus:border-[#4CA3DD] p-2.5">
                    ...
                    <select id="filter-sort" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#4CA3DD] focus:border-[#4CA3DD] p-2.5">
                        ...
                        <option value="students-asc">Étudiants (croissant)</option>
                        <option value="students-desc">Étudiants (décroissant)</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <!-- Pagination -->
        @if(isset($centers) && method_exists($centers, 'hasPages') && $centers->hasPages())
            <div class="flex flex-col sm:flex-row justify-between items-center mt-6 px-6 py-4 bg-white border-t border-gray-200">
                <div class="pagination-info mb-4 sm:mb-0">
                    Affichage de <span>{{ $centers->firstItem() ?? 0 }}</span> à <span>{{ $centers->lastItem() ?? 0 }}</span> sur <span>{{ $centers->total() }}</span> centres
                </div>
                <div class="pagination-controls">
                    {{ $centers->links('vendor.pagination.tailwind') }}
                </div>
            </div>
        @endif
    </div>
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const academyFilter = document.getElementById('filter-academy');
            ...
            if (academyFilter) {
                academyFilter.addEventListener('change', function() {
                    console.log('Filtre académie:', this.value);
                    // Logique de filtrage à implémenter
                });
            }
