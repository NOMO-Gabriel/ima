...
@section('title', 'Modifier la phase')
...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('admin.phases.show', ['locale' => app()->getLocale(), 'phase' => $phase]) }}"
                       class="ml-1 text-sm font-medium transition-all duration-200 md:ml-2"
                       :class="darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-700 hover:text-[#4CA3DD]'">
                        Phase #{{ $phase->id }}
                    </a>
    ...
    @can('phase.update', $phase)
    <div class="shadow-md rounded-lg p-6 mb-8" :class="darkMode ? 'bg-[#334155]' : 'bg-white'">
        <!-- En-tête avec titre -->
        <div class="flex items-center mb-8">
            <div class="w-10 h-10 rounded-full bg-[#FBBF24] flex items-center justify-center text-white mr-4">
            ...
            <div>
                ...
                <p class="text-sm mt-1" :class="darkMode ? 'text-gray-400' : 'text-gray-600'">
                    Phase créée le {{ $phase->created_at->format('d/m/Y à H:i') }}
                </p>
            </div>
        </div>
        ...
        <!-- Formulaire -->
        <form action="{{ route('admin.phases.update', [app()->getLocale(), $phase]) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="border-l-4 border-[#FBBF24] p-4 rounded"
                 :class="darkMode ? 'bg-yellow-900/20' : 'bg-yellow-50'">
                <div class="flex">
                    <div class="flex-shrink-0">
                        <svg class="h-5 w-5 text-[#FBBF24]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                            <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                        </svg>
                    </div>
                    <div class="ml-3">
                        <h3 class="text-sm font-medium" :class="darkMode ? 'text-yellow-300' : 'text-yellow-800'">
                            Modification d'une phase existante
                        </h3>
                        <div class="mt-2 text-sm" :class="darkMode ? 'text-yellow-300' : 'text-yellow-700'">
                            <p>Cette phase est actuellement définie du <strong>{{ \Carbon\Carbon::parse($phase->start)->format('d/m/Y') }}</strong> au <strong>{{ \Carbon\Carbon::parse($phase->end)->format('d/m/Y') }}</strong>.</p>
                            <p class="mt-1">Vérifiez que les nouvelles dates ne créent pas de conflits avec les formations déjà programmées.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t"
                ...
                <button type="submit" class="inline-flex justify-center items-center px-6 py-3 bg-[#FBBF24] hover:bg-[#F59E0B] text-white font-medium rounded-lg transition duration-150 ease-in-out shadow-md hover:shadow-lg">
