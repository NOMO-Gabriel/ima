...
@section('title', 'Créer un Concours Blanc')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Nouveau Concours</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    @can('gestion.exam.create')
        <div class="max-w-4xl mx-auto" x-data="{ darkMode: localStorage.getItem('theme') === 'dark' }">
                ...
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-lg transition-colors mt-1" :class="darkMode ? 'text-gray-400' : 'text-gray-600'">
                            Organisez une nouvelle évaluation pour vos étudiants
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('admin.mock-exams.store', ['locale' => app()->getLocale()]) }}" method="POST"
                  class="space-y-8" id="create-exam-form">
                @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-xs transition-colors" :class="darkMode ? 'text-gray-400' : 'text-gray-500'">
                                Choisissez la date et l'heure de début du concours
                            </p>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="type"
                                ...
                                <option value="QCM" {{ old('type') == 'QCM' ? 'selected' : '' }}>
                                    📝 QCM (Questions à Choix Multiples)
                                </option>
                                <option value="REDACTION" {{ old('type') == 'REDACTION' ? 'selected' : '' }}>
                                    ✍️ Rédaction (Épreuves écrites)
                                </option>
                                <option value="MIX" {{ old('type') == 'MIX' ? 'selected' : '' }}>
                                    🔄 Mixte (QCM + Rédaction)
                                </option>
                            </select>
                            @error('type')
                            ...
                            <div id="type-description" class="text-xs transition-colors" :class="darkMode ? 'text-gray-400' : 'text-gray-500'">
                                Sélectionnez le format d'évaluation souhaité
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between text-xs transition-colors" :class="darkMode ? 'text-gray-400' : 'text-gray-500'">
                                ...
                                <span id="duration-display">2h 00min</span>
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-sm text-red-500">{{ $message }}</p>
                            @enderror
                            <div id="formation-info" class="text-xs transition-colors hidden" :class="darkMode ? 'text-gray-400' : 'text-gray-500'">
                    ...
                    <div class="flex items-center mb-6">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm transition-colors" :class="darkMode ? 'text-gray-400' : 'text-gray-500'">
                                Sélectionnez les cours à évaluer (optionnel)
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        @if($courses->isEmpty())
                            <div class="text-center py-8">
                                ...
                                <p class="text-sm transition-colors mt-2" :class="darkMode ? 'text-gray-500' : 'text-gray-400'">
                                    Vous pouvez créer le concours sans cours associés
                                </p>
                            </div>
                        @endif
                        ...
                        <div class="flex items-center justify-between p-3 border-t" :class="darkMode ? 'border-gray-600' : 'border-gray-200'">
                            <span class="text-sm transition-colors" :class="darkMode ? 'text-gray-400' : 'text-gray-600'">
                                <span id="selected-count">0</span> cours sélectionné(s)
                            </span>
        ...
        <div class="max-w-2xl mx-auto text-center py-12" x-data="{ darkMode: localStorage.getItem('theme') === 'dark' }">
            <div class="rounded-lg p-8 shadow-md transition-colors"
                ...
                <p class="transition-colors mb-6" :class="darkMode ? 'text-gray-400' : 'text-gray-600'">
                    Vous n'avez pas les permissions nécessaires pour créer un concours blanc.
                </p>
                <a href="{{ route('admin.mock-exams.index', ['locale' => app()->getLocale()]) }}"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            typeSelect.addEventListener('change', function() {
                ...
                if (selectedType && typeDescriptions[selectedType]) {
                    ...
                    typeDescription.textContent = 'Sélectionnez le format d\'évaluation souhaité';
                }
            });
            ...
            formationSelect.addEventListener('change', function() {
                ...
                if (selectedOption.value) {
                    ...
                    let infoHtml = '';
                    if (description) {
                        infoHtml += `<div class="mb-1"><strong>Description:</strong> ${description}</div>`;
                    }
                    if (hours) {
                        infoHtml += `<div class="mb-1"><strong>Durée totale:</strong> ${hours} heures</div>`;
                    }
                    if (price) {
                        infoHtml += `<div><strong>Prix:</strong> ${price} FCFA</div>`;
                    }

                    if (infoHtml) {
                        formationInfo.innerHTML = infoHtml;
                        formationInfo.classList.remove('hidden');
                    } else {
                        formationInfo.classList.add('hidden');
                    }
                } else {
                    formationInfo.classList.add('hidden');
                }
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (!confirm('Êtes-vous sûr de vouloir créer ce concours blanc ?')) {
                    e.preventDefault();
                    return false;
                }
            });

            // Auto-remplissage de la date (maintenant + 1 jour)
            if (!document.getElementById('date').value) {
                const tomorrow = new Date();
                tomorrow.setDate(tomorrow.getDate() + 1);
                tomorrow.setHours(9, 0, 0, 0); // 9h00 par défaut
