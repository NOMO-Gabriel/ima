...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('admin.entrance-exams.show', ['locale' => app()->getLocale(), 'entrance_exam' => $entrance_exam->id]) }}"
                       class="ml-1 text-sm font-medium transition-all duration-200 md:ml-2"
                       :class="darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-700 hover:text-[#4CA3DD]'">
                        {{ $entrance_exam->code }}
                    </a>
    ...
    <div class="shadow-md rounded-lg p-6 mb-8" :class="darkMode ? 'bg-[#334155]' : 'bg-white'">
            ...
            <div class="flex items-center mb-4 sm:mb-0">
                ...
                <div>
                    ...
                    <p class="text-sm mt-1" :class="darkMode ? 'text-gray-400' : 'text-gray-600'">
                        Modifiez les informations du concours <span class="font-medium">{{ $entrance_exam->code }}</span>
                    </p>
                </div>
            </div>

            <!-- Badge du concours actuel -->
        ...
        <div class="mb-8 border rounded-lg p-4" :class="darkMode ? 'border-gray-600 bg-gray-800/30' : 'border-gray-200 bg-gray-50'">
            <h3 class="text-lg font-medium mb-4 flex items-center" :class="darkMode ? 'text-gray-200' : 'text-gray-800'">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div class="p-3 rounded border" :class="darkMode ? 'border-gray-600 bg-gray-700' : 'border-gray-200 bg-white'">
                    <span class="text-sm font-medium" :class="darkMode ? 'text-gray-300' : 'text-gray-600'">Code actuel :</span>
                    <p class="mt-1 font-mono text-sm font-medium text-blue-600" :class="darkMode ? 'text-blue-400' : 'text-blue-600'">
                        {{ $entrance_exam->code }}
                    </p>
                </div>
                <div class="p-3 rounded border" :class="darkMode ? 'border-gray-600 bg-gray-700' : 'border-gray-200 bg-white'">
                    <span class="text-sm font-medium" :class="darkMode ? 'text-gray-300' : 'text-gray-600'">Nom actuel :</span>
                    <p class="mt-1 text-sm font-medium" :class="darkMode ? 'text-gray-100' : 'text-gray-900'">
                        {{ $entrance_exam->name }}
                    </p>
                </div>
            </div>
        ...
        <!-- Formulaire -->
        <form method="POST" action="{{ route('admin.entrance-exams.update', [app()->getLocale(), $entrance_exam]) }}" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div class="form-group">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code', $entrance_exam->code) }}" required
                        ...
                        <div id="code-change-indicator" class="hidden absolute inset-y-0 right-0 pr-3 flex items-center">
                ...
                <div class="form-group">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $entrance_exam->name) }}" required
                               class="block w-full pl-10 pr-4 py-3 border rounded-lg shadow-sm focus:ring-[#4CA3DD] focus:border-[#4CA3DD] sm:text-sm transition duration-150 ease-in-out"
                               :class="darkMode ? 'bg-[#2C3E50] text-gray-300 border-gray-600' : 'bg-white text-gray-700 border-gray-300'" />
                        <!-- Indicateur de changement -->
                        <div id="name-change-indicator" class="hidden absolute inset-y-0 right-0 pr-3 flex items-center">
                            <svg class="h-5 w-5 text-orange-500" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                            </svg>
                        </div>
            ...
            <!-- Zone d'information sur les modifications -->
            <div class="border-l-4 border-orange-500 p-4 rounded"
                 :class="darkMode ? 'bg-orange-900/20' : 'bg-orange-50'">
                <div class="flex">
                    <div class="flex-shrink-0">
                        <svg class="h-5 w-5 text-orange-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                            <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                        </svg>
                    </div>
                    <div class="ml-3">
                        <p class="text-sm" :class="darkMode ? 'text-orange-300' : 'text-orange-700'">
                            <strong>Attention :</strong> La modification du code ou du nom peut impacter les données associées au concours (candidatures, résultats, etc.). Assurez-vous que ces changements n'affecteront pas les processus en cours.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t"
                ...
                <button type="submit" id="submit-btn"
                    ...
                    <span id="submit-text">Modifier</span>
                </button>

                <a href="{{ route('admin.entrance-exams.index', ['locale' => app()->getLocale(), 'entrance_exam' => $entrance_exam->id]) }}"
